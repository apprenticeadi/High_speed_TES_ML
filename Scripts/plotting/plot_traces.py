...
# plot together
fig, axs = plt.subplot_mosaic([['(a)', '(b)'], ['(d)', '(c)']],
                              figsize=(12, 6), height_ratios=[3,4])
plt.subplots_adjust(hspace=0.35, wspace=0.)

for label, ax in axs.items():
    ax.set_title(label, fontfamily='serif', loc='left')

ax1 = axs['(a)']
for i in range(to_show):
    ax1.plot(data100_to_plot[i], color='blue', alpha=0.05)
...
ax2 = axs['(b)']
for i in range(to_show):
    ax2.plot(data_high_to_plot[i], color='blue', alpha=0.05)
ax2.set_xlabel('Samples')
ax2.set_ylim(-1000, 20000)
ax2.set_yticks([])
ax2.set_xlim(0, 2*period)

ax3 = axs['(c)']
ax4 = axs['(d)']

refTraces.fit_histogram(plot=True, fig_name=None, coloring=False, indexing=True,
                        hist_color='blue', plot_fit=False)
plt.ylim(0, 320)
pos3 = ax3.get_position()
ax3.set_position(ax4.get_position())
ax4.set_position(pos3)

ax3.hist(refTraces.inner_products(), bins=refTraces.num_bins, color='blue', alpha=1.)
ax3.set_xlabel('Inner product')
ax3.set_ylabel('Counts')
ax3.set_ylim(0, 320)



ax4.hist(highTraces.inner_products(), bins=highTraces.num_bins, color='blue', alpha=1.)
ax4.set_xlabel('Inner product')
ax4.set_ylim(0, 320)
ax4.set_yticks([])

fig.savefig(DFUtils.create_filename(save_dir + rf'\{sub_dir}_100_vs_{high_freq}kHz_traces.pdf'))