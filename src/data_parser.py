...
class DataParser(object):
    ...
    def find_trigger(data, samples_per_trace, method='troughs', n_troughs=10):
        ...
        if method == 'troughs':
            for i in range(len(data)):
                ...
                triggers[i] = int(np.median(troughs[:n_troughs] % samples_per_trace))
    ...
    def parse_data(self, frequency, interpolated=False, triggered=True, **kwargs):
        ...
        f = frequency // 100
        ex_samples, data_intp = self.interpolate_data(data_raw, f)
        ...
        if triggered:
            if frequency < 300:
                ...
                triggers = self.find_trigger(data_intp, samples_per_trace=500)
        ...
        if interpolated:
            intp_traces = self.chop_traces(data_intp, samples_per_trace=500, trigger=trigger)
            return intp_traces

        else:
            # give back the un-interpolated data.
            samples_per_trace = int(5e4 / frequency)
            intp_samples = self.chop_traces(ex_samples, samples_per_trace=500, trigger=trigger)
...
if __name__ == '__main__':
    ...
    f = frequency // 100
    ex_samples, data_intp = dataParser.interpolate_data(data_raw, f)
    ...
    triggers = dataParser.find_trigger(data_intp, samples_per_trace=500)
    for i, ax in enumerate(axs):
        ax.axvline(ex_samples[triggers[i]], ymin=0, ymax=max(data_intp[i]), color='gray', linestyle='dashed')

    raw_traces = dataParser.chop_traces(data_raw, samples_per_trace=int(5e4 // frequency), trigger=0)
